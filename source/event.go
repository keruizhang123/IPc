package source

import (
	"github.com/KRZ/common/discovery"
)

var eventChan chan *Event

// set the proprity of the eventchannel
func EventChan() <-chan *Event {
	return eventChan
}

type EventType string

const (
	AddNodeEvent EventType = "addNode"
	DelNodeEvent EventType = "delNode"
)

// set the event struct
type Event struct {
	Type         EventType
	IP           string
	Port         string
	ConnectNum   float64
	MessageBytes float64
}

// set the new evemnt to deal with discovery
func NewEvent(ed *discovery.EndpointInfo) *Event {
	if ed == nil || ed.MetaData == nil {
		return nil
	}
	var connNum, msgBytes float64
	if data, ok := ed.MetaData["connect_num"]; ok {
		connNum = data.(float64) //If something goes wrong, panic should expose the error here.
	}
	if data, ok := ed.MetaData["message_bytes"]; ok {
		msgBytes = data.(float64) // If something goes wrong, panic should expose the error here.
	}
	return &Event{
		Type:         AddNodeEvent,
		IP:           ed.IP,
		Port:         ed.Port,
		ConnectNum:   connNum,
		MessageBytes: msgBytes,
	}
}

// /
// func (nd *Event) Key() string {
// 	return fmt.Sprintf("%s:%s", nd.IP, nd.Port)
// }
